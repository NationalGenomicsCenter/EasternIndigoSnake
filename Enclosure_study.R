...
################################################################################
###
### Leah Samuels 12/01/2024
### Script for Persistence of Reptile DNA in a Terrestrial Substrate:
### A Case Study Using the Eastern Indigo Snake
###
################################################################################
...
### two samples were 9 days not 10 days, adjusting naming
data$id_tag[(which(data$id_tag[] == "627_D4_10 DAY"))] <- "627_D4_9 DAY"
data$id_tag[(which(data$id_tag[] == "627_D3_10 DAY"))] <- "627_D3_9 DAY"

### first and last day of the weather loggers doesn't have full weather and
...
weather$average_light[which(weather$date == "08/18/2023")
  ]<- mean(weather$average_light[2:nrow(weather)])
weather$max_light[which(weather$date == "08/18/2023")
  ]<- mean(weather$max_light[2:nrow(weather)])
...
data2[c('snake_id', 'quadrat', 'sample_time', 'method')
      ] <- str_split_fixed(data2$id, '_', 4)
...
### get rid of duplicates in data set (samples that were rerun using sample with
### higher quant)
...
for (i in 1:length(data2$quadrat)){
  data2$time_taken[i] <- schedule[
    which(schedule$quadrat[] == data2$quadrat[i]), which(colnames(schedule
      ) == data2$sample_time[i])]
}

### adding dates to samples with adjusted times
data2$time_taken[which(data2$id == "627_D4_9 DAY")] <- "8/29/2023"
data2$time_taken[which(data2$id == "627_D3_9 DAY")] <- "8/29/2023"
...
for (i in 1:length(data2$id)){
  ...
  data2$median_rain[i] <- median(total$mm_rain)
}

data2['day_rain'] <- NA
for (i in 1:length(data2$day_rain)){
  for (j in 1:length(weather$date)){
...
amp2 <- amp
### putting 6 DAY and 9 DAY in 7 and 10 DAY categories
amp2$sample_time[(which(amp2$sample_time[] == "6 DAY"))] <- "7 DAY"
amp2$sample_time[(which(amp2$sample_time[] == "9 DAY"))] <- "10 DAY"
...
hist(data2$average_light)
mean(data2$average_light)
median(data2$average_light)

hist(data2$average_temp)
mean(data2$average_temp)
median(data2$average_temp)

hist(data2$average_rain)
mean(data2$average_rain)
median(data2$average_rain)

###rain not normal dist. 


#visualize snake comparison
xyplot(quant ~ int_sample | factor(snake_id), type=c("p","r"), pch = 19,
       xlab = "Sample time taken (hr)", col = 1, ylab = "Quant", layout = c(4,2), 
       data = data3)
...
###collinearity check of environmental covariates
...
### scaled data had same AIC and AUC as unscaled

### add weight for snakes
data2['weight'] <- NA
data2$weight[(which(data3$snake_id[] == "271"))] <- 2800
data2$weight[(which(data3$snake_id[]== "416"))] <- 2450
data2$weight[(which(data3$snake_id[]== "565"))] <- 2340
data2$weight[(which(data3$snake_id[]== "617"))] <- 1680
data2$weight[(which(data3$snake_id[]== "618"))] <- 2620
data2$weight[(which(data3$snake_id[]== "620"))] <- 3750
data2$weight[(which(data3$snake_id[]== "621"))] <- 1770
data2$weight[(which(data3$snake_id[]== "627"))] <- 3240
...
dates <- unique(data2$time_taken)

### Plot for temperature
plot(y=c(min(0),max(weather$max_temp)),x=c(min(weather$date),max(weather$date)),
     type="n", ylab="Temp (˚C)", las = 2, xaxt = 'n')
abline(v=dates, col="gray")
polygon(x=c(xcoords,rev(xcoords)),y=c(weather$max_temp,weather$average_temp
                                      ),col=rangecolor,border=NA)
polygon(x=c(xcoords,rev(xcoords)),y=c(weather$min_temp,weather$average_temp
                                      ),col=rangecolor,border=NA)
...
axis(2, at = c(0,10000,20000,30000, 40000, 50000, 60000), labels = c(
  "0", "10k", "20k", "30k","40k", "50k","60k"), las =2)
polygon(x=c(xcoords,rev(xcoords)),y=c(weather$max_light,weather$average_light
                                      ),col=rangecolor,border=NA)
polygon(x=c(xcoords,rev(xcoords)),y=c(weather$min_light,weather$average_light
                                      ),col=rangecolor,border=NA)
...
axis(side = 1, at = seq(min(weather$date),max(weather$date), by =4), 
     cex = 0.5,labels = c("Aug 18","Aug 22", "Aug 26", "Aug 30", "Sep 3", 
                          "Sep 7","Sep 11"))
...
plot1 <- ggplot(data3, aes(x=factor(exposure_time, level = c(
  "100 seconds", "15 minutes","1 hour(s)", "2 hour(s)")), y=quant)) + 
  geom_violin(fill = "darkgray") + xlab("Time in enclosure")+ ylab(
    "EIS mtDNA copies/rxn") +  theme_classic() + theme(
...
plot2 <- ggplot(data3, aes(x=factor(sample_time, level = c("1 HR", "4 HR",
  "24 HR","48 HR", "72 HR", "7 DAY", "10 DAY")), y=quant)) + geom_violin(
  fill = "darkgray") + xlab("Sand collection")+ theme_classic() + theme( 
  axis.text.y=element_blank(), axis.ticks.y=element_blank(), 
  axis.title.y=element_blank(), axis.text = element_text(colour = "black"
  ), axis.line = element_blank(), panel.border = element_rect(
  colour = "black", fill=NA, linewidth=0.5)) + geom_jitter(size = 0.9,
...
glm_1 <- glm(amp ~ int_exposure + int_sample, family = binomial(link = "logit"),
      data = data2)
glmer_1 <- glmer(amp ~ int_exposure + int_sample + (1 | snake_id), family 
     = binomial(link = "logit"), data = data2)
glmer_2 <- glmer(amp ~ int_exposure + (1| snake_id) , family = binomial(
    link = "logit"), data = data2)
glmer_3 <- glmer(amp ~  int_sample + (1|snake_id), family = binomial(link =
    "logit"), data = data2)

###compare base models
anova(glm_1,glmer_1,glmer_2,glmer_3, test  = "LRT")
AIC(glm_1,glmer_1,glmer_2,glmer_3)
AICc(glmer_1)
AICc(glmer_2)
AICc(glmer_3)
summary(glmer_1)
...
#confint(glmer_1, level = 0.95, method = "boot", nsim = 500)

###############################################################################
### adding environmental covariates, rain and temp collinear 


### light and temp

### comparing to top base model glmer_1
glmer_1 <- glmer(amp ~ int_exposure + int_sample + (1 | snake_id), family 
       = binomial(link = "logit"), data = data2)
glmer_2 <- glmer(amp ~ int_exposure + int_sample + average_temp + average_light
      + (1|snake_id), family = binomial(link = "logit"), data = data2)
glmer_temp <- glmer(amp ~ int_exposure + int_sample + 
      average_temp + (1|snake_id), family = binomial(link = "logit"), data =
      data2)
glmer_light <- glmer(amp ~ int_exposure + int_sample + 
      average_light + (1|snake_id), family = binomial(link = "logit"), data =
      data2)

anova(glmer_1,glmer_2,glmer_temp,glmer_light, test = "LRT")
AIC(glmer_1,glmer_2,glmer_temp,glmer_light)
AICc(glmer_1)
AICc(glmer_2)
AICc(glmer_temp)
AICc(glmer_light)
...
### rain and light
glmer_5 <- glmer(amp ~ int_exposure + int_sample + average_light +
    average_rain + (1 | snake_id), family = binomial(link = "logit"), data = data2)
glmer_rain <- glmer(amp ~ int_exposure + int_sample + average_rain + 
    (1 | snake_id), family = binomial(link = "logit"), data = data2)

anova(glmer_1,glmer_5,glmer_light,glmer_rain, test = "LRT")
AIC(glmer_1,glmer_5,glmer_light,glmer_rain)
AICc(glmer_1)
AICc(glmer_5)
AICc(glmer_light)
AICc(glmer_rain)
...
### confidence intervals
#confint(glmer_light, devtol = Inf)
#confint(glmer_rain, level = 0.95, method = "boot", nsim = 500)
#confint(glmer_temp, level = 0.95, method = "boot", nsim = 500)
...
train <- data2[sample, ]
test <- data2[!sample, ]
test_glmer <- glmer(amp ~ int_exposure + (1|snake_id) , family = binomial(
  link = "logit"), data = train)
...
### plotting the modeled probability
data2$snake_id <- as.factor(data2$snake_id)
glmtest <- glmer(amp ~ int_sample + int_exposure + (1|snake_id), family =
                   binomial(link = "logit"), data = data2)

### creating data frames of the four exposure conditions with the random 
### intercept taken into account (8 snakes per condition)

re.hour2 <- data.frame(int_exposure = rep(120, 800),
  int_sample = rep(seq(min(data2$int_sample), max(data2$int_sample), length.out 
  = 100), length(levels(data2$snake_id))), snake_id = factor(rep(levels(
  data2$snake_id), each = 100)))
re.hour1 <- data.frame(int_exposure = rep(60, 800),int_sample = rep(seq(min(
  data2$int_sample), max(data2$int_sample), length.out = 100), length(levels(
  data2$snake_id))),snake_id = factor(rep(levels(data2$snake_id), each = 100)))
re.min15 <- data.frame(int_exposure = rep(15, 800),int_sample = rep(seq(min(
  data2$int_sample), max(data2$int_sample), length.out = 100), length(levels(
  data2$snake_id))),snake_id = factor(rep(levels(data2$snake_id), each = 100)))
re.min2 <- data.frame(int_exposure = rep(1.66, 800), int_sample = rep(seq(min(
  data2$int_sample), max(data2$int_sample), length.out = 100), length(levels(
    data2$snake_id))),snake_id = factor(rep(levels(data2$snake_id), each = 100)))

re.hour2$pred <- predict(glmtest,re.hour2, type = "response", 
                         re.form = ~(1|snake_id))
re.hour1$pred <- predict(glmtest,re.hour1, type = "response", 
                         re.form = ~(1|snake_id))
re.min15$pred <- predict(glmtest,re.min15, type = "response",
                         re.form = ~(1|snake_id))
re.min2$pred <- predict(glmtest,re.min2, type = "response",
                        re.form = ~(1|snake_id))

### creating data frames of four exposure conditions without random intercept 
### taken into account
...
plot(NULL, NULL, xlim = c(0, 240), ylim = c(0,1), las = 1, 
     xlab = "Sand collection time (hrs)", ylab = "Pr (detection)")

list <- unique(data2$snake_id)
for (i in 1:length(list)){
  lines(pred ~ int_sample, data = subset(re.hour2, snake_id == list[i]), 
        lwd = 1, col = rgb(1,0,0, alpha=0.15))
}
for (i in 1:length(list)){
  lines(pred ~ int_sample, data = subset(re.hour1, snake_id == list[i]),
        lwd = 1, col = rgb(0,1,1, alpha=0.15))
}
for (i in 1:length(list)){
  lines(pred ~ int_sample, data = subset(re.min15, snake_id == list[i]),
        lwd = 1, col = rgb(1,0.5,0, alpha=0.15))
}
for (i in 1:length(list)){
  lines(pred ~ int_sample, data = subset(re.min2, snake_id == list[i]), 
        lwd = 1, col = rgb(0,0,1, alpha=0.15))
}
...
legend("topright", cex = 0.65, bty = "n", lwd = 2, col = 
    c("darkblue","orange", "darkturquoise", "red"),legend = c(
    "100 s", "15 min", "1 hr", "2 hr"), title = "Time in enclosure",)
