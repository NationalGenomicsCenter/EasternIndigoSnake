...
plot(NULL, NULL, xlim = c(0, 7000), ylim = c(0,1), las = 1, xlab = "Cost ($)", ylab = "Pr (detection) camera") +
  lines(burrow_camera_df$cost,burrow_camera_df$det_prob, col = "darkred", lwd = 2, type = 's') + 
  lines(ground_camera_df$cost,ground_camera_df$det_prob, col = "salmon1", lwd = 2, type = 's') +
  abline(h = 0.95, lty =2)
legend("bottomleft", cex = 1, bty = "n", lwd = 2, col = c( "darkred","salmon1"
),legend = c("Burrow camera", "Drift fence"))


plot(NULL, NULL, xlim = c(0, 5000), ylim = c(0,1), las = 1, xlab = "Cost ($)", ylab = "Pr (detection) eDNA")
lines(burrow_edna_df$cost,burrow_edna_df$det_prob, col = "darkgreen", lwd = 2)
lines(ground_edna_df$cost,ground_edna_df$det_prob, col = "lightgreen", lwd = 2)
abline(h = 0.95, lty =2)
legend("bottomleft", cex = 1, bty = "n", lwd = 2, col = c( "darkgreen","lightgreen"
),legend = c("Burrow eDNA", "Ground eDNA"))

plot(NULL, NULL, xlim = c(0, 5000), ylim = c(0,1), las = 1, xlab = "Cost($)", ylab = "Pr (detection) search")
lines(search_high$cost,search_high$det_prob, col = "lightskyblue1", lwd = 2, type = 's')
lines(search_low$cost, search_low$det_prob, col = 'royalblue4', lwd = 2, type = 's')
abline(h = 0.95, lty =2)
legend("bottomleft", cex = 1, bty = "n", lwd = 2, col = c( "royalblue4","lightskyblue1"
),legend = c("Search lower cost", "Search upper cost"))
...
data_new <- genpresEVmm(
  N = 30,                   # number of sites 
  K = 6,                     # number of surveys
  psi = 1,                 # true occupancy prob.
  p = c(0.3,0.4,0.3,0.4,0.3,0.4), # true detection prob. method survey
  theta = c(0.4,0.4,0.4,0.4) ,      # local occupancy
  methods = 2
)

data1 <- data_new$frq * data_new$hst
rownames(data1$hst) <- rownames(data)
...
mmmod_sim <- occMod(    
  ...
  model = list(       
    psi ~ 1,             #  occupancy constant for all sites
    theta ~ 1,      #  local occupancy site or 1
    p ~ METHOD) #  detection different for each method and fence vs burrow    
)   

ggdata3 <- rbind(
  unique(mmmod_sim$real$psi),
  unique(mmmod_sim$real$p),
  unique(mmmod_sim$real$theta)
)
...
sim2 <- data.frame(matrix(nrow = 30, ncol = 9))
colnames(sim2) <- c('theta','theta1','theta2','T1M1','T1M2','T2M1','T2M2','T3M1','T3M2')
sim2[] <- 0
sim2$theta[0:15] <- sample(c(0,1), 15,replace = TRUE, prob = c(0.6,0.4))
sim2$theta[16:30] <- sample(c(0,1), 15,replace = TRUE, prob = c(0.8,0.2))

sim2$theta1[0:15] <- sample(c(0,1), 15,replace = TRUE, prob = c(0.6,0.4))
sim2$theta1[16:30] <- sample(c(0,1), 15,replace = TRUE, prob = c(0.8,0.2))

sim2$theta2[0:15] <- sample(c(0,1), 15,replace = TRUE, prob = c(0.6,0.4))
sim2$theta2[16:30] <- sample(c(0,1), 15,replace = TRUE, prob = c(0.8,0.2))


#camera = 0.5, eDNA = 0.3
for(i in 1:nrow(sim2)){
  if(sim2$theta[i] == 1){
    sim2[i,'T1M1'] <- sample(c(0,1),1, replace = TRUE,prob = c(0.7,0.3))
    sim2[i,'T1M2'] <- sample(c(0,1),1, replace = TRUE,prob = c(0.5,0.5))
  }
  else if(sim2$theta1[i] == 1){
    sim2[i,'T2M1'] <- sample(c(0,1),1, replace = TRUE,prob = c(0.7,0.3))
    sim2[i,'T2M2'] <- sample(c(0,1),1, replace = TRUE,prob = c(0.5,0.5))
  }
  else if (sim2$theta2[i] == 1){
    sim2[i,'T3M1'] <- sample(c(0,1),1, replace = TRUE,prob = c(0.7,0.3))
    sim2[i,'T3M2'] <- sample(c(0,1),1, replace = TRUE,prob = c(0.5,0.5))
  }
}

# for (i in 1:ncol(sim2)){
#   if (grepl('M1', colnames(sim2)[i])){
#     for (j in 1:nrow(sim2)){
#     if (sim2$theta[j] == 1){
#       sim2[j,i] <- sample(c(0,1),1, replace = TRUE,prob = c(0.7,0.3))
#     }
#     }
# 
#   } else if (grepl('M2',colnames(sim2)[i])){
#     for (j in 1:nrow(sim2)){
#       if (sim2$theta[j] == 1){
#         sim2[j,i] <- sample(c(0,1),1, replace = TRUE,prob = c(0.5,0.5))
#       }
#     }
#   }
# 
...
sim_cov <- data.frame(location = c(rep('High',15),rep('Low',15)))
...
mmmod_sim <- occMod(    
  ...
  model = list(       
    psi ~ 1,             #  occupancy constant for all sites
    theta ~ location,      #  local occupancy site or 1
    p ~ METHOD) #  detection different for each method and fence vs burrow       
)   
...
if(length(ggdata_sim$est[rownames(ggdata_sim) =='theta1_1'])>0){
theta_location_1[l] <- ggdata_sim$est[rownames(ggdata_sim) =='theta1_1']}

if(length(ggdata_sim$est[rownames(ggdata_sim) =='theta1_16'])>0){
theta_location_2[l] <- ggdata_sim$est[rownames(ggdata_sim) =='theta1_16']}

if(length(ggdata_sim$est[rownames(ggdata_sim) =='p1(1-1)_1_unit1']) > 0){
p_m1[l] <- ggdata_sim$est[rownames(ggdata_sim) =='p1(1-1)_1_unit1']}

if(length(ggdata_sim$est[rownames(ggdata_sim) =='p2(1-1)_1_unit1']) > 0){
p_m2[l] <- ggdata_sim$est[rownames(ggdata_sim) =='p2(1-1)_1_unit1']}
#coef(mmmod_sim, param = 'theta', prob = 0.95)

}

hist(theta_location_1)
hist(theta_location_2)
hist(p_m1)
hist(p_m2)
